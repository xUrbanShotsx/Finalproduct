...
const AGENTS = ["Respirable Silica","Nuisance Dust","Welding Fumes","Isocyanates","Noise","Hexavalent Chromium","Chemical Vapour","UV Radiation","Hand-Arm Vibration","Whole-Body Vibration"];
const SITES = ["Site 01","Site 02","Site 03"];
const CONTROL_METHODS = ["Elimination","Substitution","Engineering (LEV)","Administrative","PPE (Half-face)","PPE (Full-face)","PPE (P2 Mask)","PPE (Hearing)"];
...
export function ChemicalProcessRiskDrawer({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd?: (f: typeof INIT) => void }) {
  ...
  return (
    <Drawer open={open} onClose={reset} title="Add Chemical" step={1} totalSteps={1}
      ...
      <Section>
        <Row>
          <Col><Label>Worker Group *</Label><Input value={f.workerGroup} onChange={v => s("workerGroup", v)} placeholder="e.g. Concrete grinders" /></Col>
          <Col><Label>Area / Location</Label><Input value={f.area} onChange={v => s("area", v)} placeholder="e.g. Podium level" /></Col>
        </Row>
      </Section>
      <Section>
        <Row>
        ...
        <Section>
          <div className="p-3 border" style={{ background: "rgba(240,96,96,0.06)", borderColor: "#f06060" }}>
            <span className="text-[12px] font-semibold" style={{ color: "#f06060" }}>
              Exposure exceeds WES — immediately implement additional controls and notify PCBU.
            </span>
          </div>
        </Section>
      )}
      <Section>
        ...
        <Select value={f.controlMethod} onChange={v => s("controlMethod", v)} placeholder="Select control…" options={CONTROL_METHODS} />
      </Section>
      <Section>
